...
export default function ActionsMenu({label,children}:{label:string;children:React.ReactNode}){
 ...
 return <details onToggle={()=>{const details=ref.current;if(!details?.open)return;const menu=details.querySelector<HTMLDivElement>('[data-actions-menu]');if(menu){const box=details.getBoundingClientRect();const width=menu.getBoundingClientRect().width;menu.style.left=`${Math.max(8,Math.min(box.right-width,window.innerWidth-width-8))-box.left}px`;}}} ref={ref} className="relative" onKeyDown={e=>{if(e.key==='Escape'&&ref.current?.open){e.preventDefault();e.stopPropagation();ref.current.open=false;ref.current.querySelector('summary')?.focus();}}}>
