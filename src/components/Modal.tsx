...
export default function Modal({title,onClose,busy=false,side=false,children}:{title:string;onClose:()=>void;busy?:boolean;side?:boolean;children:React.ReactNode}){
 const ref=useRef<HTMLDialogElement>(null);const headingId=useId();const originals=useRef(new WeakMap<Field,string>());
 const mounted=useSyncExternalStore(subscribe,()=>true,()=>false);
 function remember(target:EventTarget){if(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement||target instanceof HTMLSelectElement){if(!originals.current.has(target))originals.current.set(target,fieldValue(target));}}
 function dirty(){return [...(ref.current?.querySelectorAll<Field>('form input, form textarea, form select')||[])].some(field=>!field.closest('[data-instant-save]')&&fieldValue(field)!==(originals.current.get(field)??defaultValue(field)));}
 ...
 return createPortal(<dialog ref={ref} className={`task-dialog ${side?'task-drawer':''}`} aria-labelledby={headingId} onFocusCapture={e=>remember(e.target)}
