...
export default function TaskSummary({ tasks, today }: { tasks: SummaryTask[]; today: string }) {
  return <section aria-label="Task summary" className="grid grid-cols-2 gap-3 lg:grid-cols-4">
    {summaryFilters.map(filter => <Link key={filter.id} href={`/dashboard/tasks?summary=${filter.id}`} className="rounded-xl border border-gray-200 bg-surface p-5 hover:border-blue-400 transition">
