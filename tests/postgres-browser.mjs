...
try{
  ...
  const nav=admin.locator('aside').getByRole('navigation',{name:'Workspace',exact:true});await expect(nav.getByRole('link')).toHaveText(['Search','Dashboard','Projects','My Tasks','All Tasks','Calendar','Reports','Templates','Archive','Workload','Team Users']);await expect(nav.getByRole('link',{name:'Search',exact:true})).toBeVisible();
