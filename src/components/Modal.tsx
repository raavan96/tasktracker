...
export default function Modal({title,onClose,busy=false,side=false,children}:{title:string;onClose:()=>void;busy?:boolean;side?:boolean;children:React.ReactNode}){
 ...
 return createPortal(<dialog ref={ref} className={`task-dialog ${side?'task-drawer':''}`} aria-labelledby={headingId} onFocusCapture={e=>remember(e.target)} onPointerDownCapture={e=>remember(e.target)} onChangeCapture={e=>edited(e.target)}
