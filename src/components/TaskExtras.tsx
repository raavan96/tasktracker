...
export default function TaskExtras({ task, tasks, members, canEdit, view = 'details' }: { task:Task; tasks:Task[]; members:Member[]; canEdit:boolean; view?: 'details' | 'updates' }) {
  ...
  return <div className="space-y-6 border-t py-5">
    ...
    <div hidden={view !== 'details'} className="space-y-6"><section><h3 className="mb-3 font-semibold">Checklist <span className="text-sm text-gray-500">{data.checklist?.filter(c=>c.completed).length}/{data.checklist?.length}</span></h3>
      ...
      {canEdit&&task.status!=='done'&&<form className="flex gap-2 mt-2" onSubmit={async e=>{e.preventDefault();const form=e.currentTarget;if(await run(()=>saveChecklist(task.id,task.project_id,String(new FormData(form).get('title')||''))))form.reset();}}><input name="title" aria-label="Checklist item" required maxLength={300} placeholder="Add a checklist item" className="min-w-0 flex-1 rounded-lg border px-3 py-2 text-sm"/><button disabled={busy} className="rounded-lg border px-3 text-sm">Add</button></form>}
    </section>
    <section><h3 className="mb-3 font-semibold">Waiting on</h3>{!data.dependencies?.length&&<p className="text-sm text-gray-500">No dependencies.</p>}
      ...
      {canEdit&&task.status!=='done'&&<form className="flex gap-2 mt-2" onSubmit={async e=>{e.preventDefault();const form=e.currentTarget;const id=String(new FormData(form).get('dependency'));if(await run(()=>setDependency(task.id,task.project_id,id)))form.reset();}}><select name="dependency" aria-label="Dependency task" required className="min-w-0 flex-1 rounded-lg border p-2 text-sm"><option value="">Choose prerequisite</option>{tasks.filter(t=>t.id!==task.id&&!data.dependencies?.some(d=>d.depends_on===t.id)).map(t=><option key={t.id} value={t.id}>{t.title}</option>)}</select><button disabled={busy} className="rounded-lg border px-3 text-sm">Add</button></form>}
    </section>
