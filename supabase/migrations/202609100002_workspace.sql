...
-- Private storage; public links are never used for task files.
INSERT INTO storage.buckets(id,name,public,file_size_limit) VALUES('task-files','task-files',false,10485760) ON CONFLICT(id) DO NOTHING;
CREATE POLICY task_file_read ON storage.objects FOR SELECT TO authenticated USING(bucket_id='task-files' AND EXISTS(SELECT 1 FROM public.tasks WHERE id::text=split_part(name,'/',1) AND public.can_view_project(project_id)));
CREATE POLICY task_file_create ON storage.objects FOR INSERT TO authenticated WITH CHECK(bucket_id='task-files' AND EXISTS(SELECT 1 FROM public.tasks WHERE id::text=split_part(name,'/',1) AND public.can_work_task(id)));
CREATE POLICY task_file_delete ON storage.objects FOR DELETE TO authenticated USING(bucket_id='task-files' AND EXISTS(SELECT 1 FROM public.tasks WHERE id::text=split_part(name,'/',1) AND public.can_work_task(id)) AND (owner_id=auth.uid()::text OR public.is_admin()));
COMMIT;
