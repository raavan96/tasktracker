...
test('Every dark email template escapes content and uses trusted action links',async()=>{
 ...
 for(const kind of Object.keys(categories)){
  ...
  assert.ok(!result.html.includes('<script>'));assert.equal((result.html.match(/<img /g)||[]).length,1);assert.ok(result.html.includes('https://tasktracker.top-menus.com/branding/tasktracker-mark-v1.png'));assert.ok(!result.html.includes('<img src=x'));assert.ok(result.html.includes('&lt;script&gt;'));
