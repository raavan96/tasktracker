...
export default function WorkspaceNav({ isAdmin, mobile = false }: { isAdmin: boolean; mobile?: boolean }) {
  ...
  const navigation = <nav aria-label="Workspace" className={mobile ? "grid gap-1 py-2 text-sm font-medium" : "flex items-center gap-1 text-sm font-medium"}>
    {links.map(({ href, label, icon: Icon, active }) => <Fragment key={href}><Link onClick={() => { if(menu.current) menu.current.open = false; }} key={href} href={href} aria-label={label} data-tooltip={!mobile ? label : undefined} title={label==='Search'?'Search workspace':undefined} aria-current={active ? 'page' : undefined}
    ...
    </Link>{label==='Search'&&<DashboardLauncher />}</Fragment>)}
  </nav>;
