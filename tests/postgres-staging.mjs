...
// Email delivery is independent of in-app suppression and disabled by default.
await as(null);await db.exec(read('postgres/012_email_notifications.sql'));
const emailLogic=await import('../scripts/email/queue.mjs');
assert.equal((await rows('SELECT enabled FROM email_delivery_settings'))[0].enabled,false);
assert.equal((await rows('SELECT count(*) n FROM email_queue'))[0].n,0);
await as(owner);const ep=(await rows("SELECT create_workspace_project('Email private QA','',ARRAY[$1::uuid],true) id",[member]))[0].id;
const et=(await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids,due_date) VALUES($1,'Email task',$2,ARRAY[$3::uuid],current_date) RETURNING id",[ep,owner,member]))[0].id;
await as(member);await rows('INSERT INTO email_preferences(user_id,enabled) VALUES($1,true)',[member]);
await rows('INSERT INTO notification_preferences(user_id,assignments,mentions) VALUES($1,false,false) ON CONFLICT(user_id) DO UPDATE SET assignments=false,mentions=false',[member]);
await as(outsider);assert.equal((await rows('SELECT * FROM email_preferences')).length,0);await assert.rejects(()=>rows('SELECT * FROM email_queue'));await assert.rejects(()=>rows('UPDATE email_delivery_settings SET enabled=true'));
await as(null);
await rows("INSERT INTO notifications(user_id,task_id,title,message,dedupe_key) VALUES($1,$2,'Task assigned','Email task','email-disabled')",[member,et]);
assert.equal((await rows('SELECT count(*) n FROM email_queue'))[0].n,0);
await rows('UPDATE email_delivery_settings SET enabled=true');
async function emailNotice(key,title='Task assigned'){await rows('INSERT INTO notifications(user_id,task_id,title,message,dedupe_key) VALUES($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING',[member,et,title,'Email task',key]);}
await emailNotice('email-one');await emailNotice('email-one');
assert.equal((await rows("SELECT * FROM notifications WHERE dedupe_key='email-one'")).length,0);
assert.equal((await rows('SELECT * FROM email_queue')).length,1);
let emailCalls=0;
await emailLogic.processOneEmail(db,async mail=>{emailCalls++;assert.ok(mail.html.includes('background:#080f1b'));assert.ok(mail.subject.includes('Task assignment'));assert.equal(mail.recipient,member+'@collegedunia.com');return {httpStatus:200,body:{success:[false]}};});
assert.equal((await rows('SELECT state FROM email_queue'))[0].state,'unconfirmed');
assert.equal(await emailLogic.processOneEmail(db,async()=>{emailCalls++;}),false);assert.equal(emailCalls,1);
await emailNotice('email-withdraw');await rows('DELETE FROM project_members WHERE project_id=$1 AND user_id=$2',[ep,member]);
await emailLogic.processOneEmail(db,async()=>{throw new Error('Must not send after access removal');});
assert.equal((await rows("SELECT state FROM email_queue WHERE event_key='notice:email-withdraw'"))[0].state,'cancelled');
await rows('INSERT INTO project_members(project_id,user_id) VALUES($1,$2)',[ep,member]);
await emailNotice('email-preference');await rows('UPDATE email_preferences SET assignments=false WHERE user_id=$1',[member]);
await emailLogic.processOneEmail(db,async()=>{throw new Error('Must not send after opt-out');});
assert.equal((await rows("SELECT state FROM email_queue WHERE event_key='notice:email-preference'"))[0].state,'cancelled');
await rows('UPDATE email_preferences SET assignments=true WHERE user_id=$1',[member]);
await emailNotice('email-timeout');await emailLogic.processOneEmail(db,async()=>{throw new Error('Ambiguous connection failure');});
assert.equal((await rows("SELECT state FROM email_queue WHERE event_key='notice:email-timeout'"))[0].state,'unconfirmed');
await emailNotice('email-accepted');await emailLogic.processOneEmail(db,async()=>({httpStatus:200,body:{success:[true]}}));
assert.equal((await rows("SELECT state FROM email_queue WHERE event_key='notice:email-accepted'"))[0].state,'accepted');
await rows("SELECT queue_deadline_emails(((now() AT TIME ZONE 'Asia/Kolkata')::date+time '10:00') AT TIME ZONE 'Asia/Kolkata')");
await rows("SELECT queue_deadline_emails(((now() AT TIME ZONE 'Asia/Kolkata')::date+time '11:00') AT TIME ZONE 'Asia/Kolkata')");
assert.equal((await rows("SELECT count(*) n FROM email_queue WHERE category='deadline_digest'"))[0].n,1);
const digest=(await rows("SELECT * FROM email_queue WHERE category='deadline_digest'"))[0];
const digestEmail=await emailLogic.prepareEmail(db,digest);assert.ok(digestEmail.html.includes('Your deadlines at a glance'));
await rows('UPDATE email_preferences SET deadline_digest=false WHERE user_id=$1',[member]);assert.equal(await emailLogic.prepareEmail(db,digest),null);
console.log('Email queue: disabled by default, independent preferences, no backlog, deduplication, private access removal, opt-out, false/timeout no-retry, accepted status and daily digest passed.');
await as(null);await db.exec(read('postgres/013_member_weekly_email.sql'));
const {weeklyReportData}=await import('../scripts/email/weekly.mjs');
const week=(await rows("SELECT date_trunc('week',now() AT TIME ZONE 'Asia/Kolkata')::date::text week"))[0].week;
await rows("INSERT INTO completion_events(task_id,project_id,task_title,assignee_id,assignee_ids,occurred_at,source) VALUES($1,$2,'Email task',$3,ARRAY[$3::uuid],(($4::date-1)::timestamp AT TIME ZONE 'Asia/Kolkata'),'legacy_estimated')",[et,ep,member,week]);
await as(owner);await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids) VALUES($1,'Colleague-only personal task',$2,ARRAY[$2::uuid])",[ep,owner]);
await as(outsider);const unrelated=(await rows("SELECT create_workspace_project('Unrelated public project','',ARRAY[]::uuid[],false) id"))[0].id;
await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids) VALUES($1,'Unrelated private work details',$2,ARRAY[$2::uuid])",[unrelated,outsider]);
await as(null);
const weeklyData=await weeklyReportData(db,member,week);
assert.ok(weeklyData.projects.some(p=>p.id===ep));assert.ok(!weeklyData.projects.some(p=>p.id===unrelated));
...
await rows('UPDATE email_preferences SET weekly_report=false WHERE user_id=$1',[member]);assert.equal(await emailLogic.prepareEmail(db,weeklyJob),null);
await rows('UPDATE email_preferences SET weekly_report=true WHERE user_id=$1',[member]);
await rows('DELETE FROM project_members WHERE project_id=$1 AND user_id=$2',[ep,member]);const revokedWeekly=await emailLogic.prepareEmail(db,weeklyJob);assert.ok(!revokedWeekly?.html.includes('Email private QA'));assert.ok(!revokedWeekly?.html.includes('Email task'));
...
// Creator submission and project/self-assignment email events.
await as(null);
const beforeExperienceQueue=(await rows('SELECT count(*) n FROM email_queue'))[0].n;
await db.exec(read('postgres/016_workspace_experience.sql'));
assert.equal((await rows('SELECT count(*) n FROM email_queue'))[0].n,beforeExperienceQueue,'Do not email historical memberships');
await as(owner);
const experienceProject=(await rows("SELECT create_workspace_project('Project mail QA','',ARRAY[$1::uuid],true) id",[member]))[0].id;
await as(null);
const projectJobs=await rows("SELECT * FROM email_queue WHERE project_id=$1 AND category='project_assignment'",[experienceProject]);
assert.equal(projectJobs.length,2);
const memberProjectJob=projectJobs.find(j=>j.user_id===member);
const projectMail=await emailLogic.prepareEmail(db,memberProjectJob);
assert.ok(projectMail.subject.includes('Project assignment'));
assert.ok(projectMail.html.includes('https://tasktracker.top-menus.com/dashboard/projects/'+experienceProject));
await rows('INSERT INTO project_members(project_id,user_id) VALUES($1,$2) ON CONFLICT DO NOTHING',[experienceProject,member]);
assert.equal((await rows('SELECT count(*) n FROM email_queue WHERE project_id=$1',[experienceProject]))[0].n,2);
await rows('DELETE FROM project_members WHERE project_id=$1 AND user_id=$2',[experienceProject,member]);
assert.equal(await emailLogic.prepareEmail(db,memberProjectJob),null,'Do not email removed project members');
await rows('INSERT INTO project_members(project_id,user_id) VALUES($1,$2)',[experienceProject,member]);
await as(owner);
const selfAssigned=(await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids) VALUES($1,'Self-assigned mail QA',$2,ARRAY[$2::uuid]) RETURNING id",[experienceProject,owner]))[0].id;
const creatorDelegated=(await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids) VALUES($1,'Creator submission QA',$2,ARRAY[$3::uuid]) RETURNING id",[experienceProject,owner,member]))[0].id;
await rows("SELECT review_task($1,0,'submit','')",[creatorDelegated]);
assert.equal((await rows('SELECT status FROM tasks WHERE id=$1',[creatorDelegated]))[0].status,'in_review');
await rows("SELECT review_task($1,1,'withdraw','Need another check')",[creatorDelegated]);
await rows("SELECT review_task($1,2,'submit','')",[creatorDelegated]);
await rows("SELECT review_task($1,3,'approve','')",[creatorDelegated]);
await rows("SELECT review_task($1,0,'submit','')",[selfAssigned]);
await assert.rejects(()=>rows("SELECT review_task($1,1,'approve','')",[selfAssigned]));
await as(null);
const selfJob=(await rows("SELECT * FROM email_queue WHERE task_id=$1 AND category='assignment'",[selfAssigned]));assert.equal(selfJob.length,1);
assert.ok(await emailLogic.prepareEmail(db,selfJob[0]));
await rows("UPDATE tasks SET title='Same assignment edited' WHERE id=$1",[selfAssigned]);
assert.equal((await rows("SELECT count(*) n FROM email_queue WHERE task_id=$1 AND category='assignment'",[selfAssigned]))[0].n,1);
...
await as(null);await db.exec(read('postgres/019_workspace_history.sql'));
await as(owner);
const historyProject=(await rows("SELECT create_workspace_project('History private QA','',ARRAY[$1::uuid],true) id",[member]))[0].id;
const historyTask=(await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids) VALUES($1,'History task',$2,ARRAY[$2::uuid]) RETURNING id",[historyProject,owner]))[0].id;
await rows("UPDATE tasks SET title='History task edited',status='in_progress' WHERE id=$1",[historyTask]);
let audit=(await rows("SELECT * FROM workspace_history WHERE task_id=$1 AND action='updated'",[historyTask]))[0];
assert.equal(audit.actor_id,owner);assert.equal(audit.changes.title.before,'History task');assert.equal(audit.changes.status.after,'in_progress');
const priorAudit=(await rows('SELECT count(*) n FROM workspace_history WHERE task_id=$1',[historyTask]))[0].n;
await rows('UPDATE tasks SET updated_at=now() WHERE id=$1',[historyTask]);assert.equal((await rows('SELECT count(*) n FROM workspace_history WHERE task_id=$1',[historyTask]))[0].n,priorAudit);
await rows("INSERT INTO task_comments(task_id,author_id,content) VALUES($1,$2,'Audit remark')",[historyTask,owner]);
await rows("UPDATE task_comments SET content='Edited audit remark' WHERE task_id=$1",[historyTask]);
assert.ok((await rows("SELECT changes FROM workspace_history WHERE entity='Remarks' AND action='updated' AND task_id=$1",[historyTask]))[0].changes.content.before==='Audit remark');
await assert.rejects(()=>rows("DELETE FROM workspace_history WHERE task_id=$1",[historyTask]));
await as(outsider);assert.equal((await rows('SELECT * FROM workspace_history WHERE project_id=$1',[historyProject])).length,0);
await as(owner);await rows('DELETE FROM tasks WHERE id=$1',[historyTask]);assert.equal((await rows("SELECT count(*) n FROM workspace_history WHERE task_id=$1 AND action='deleted' AND entity='Tasks'",[historyTask]))[0].n,1);
await rows('DELETE FROM projects WHERE id=$1',[historyProject]);assert.equal((await rows('SELECT * FROM workspace_history WHERE project_id=$1',[historyProject])).length,0);
await as(admin);assert.ok((await rows("SELECT * FROM workspace_history WHERE project_id=$1 AND entity='Projects' AND action='deleted'",[historyProject])).length===1);
console.log('Workspace history: real before/after, actor, timestamp-only suppression, immutable records, private visibility and deleted record retention passed.');
await as(null);await db.exec(read('postgres/020_chat.sql'));
await as(owner);
const chatProject=(await rows("SELECT create_workspace_project('Chat QA','',ARRAY[$1::uuid],true) id",[member]))[0].id;
const chat=(await rows('SELECT open_chat($1,null) id',[chatProject]))[0].id;
const nonce='aaaaaaaa-1111-4111-8111-111111111111';
const mid=(await rows("SELECT send_chat($1,'Hello',null,ARRAY[$2::uuid],$3) id",[chat,member,nonce]))[0].id;
assert.equal((await rows("SELECT send_chat($1,'Hello',null,ARRAY[$2::uuid],$3) id",[chat,member,nonce]))[0].id,mid);
await assert.rejects(()=>rows("SELECT send_chat($1,'Changed',null,'{}',$2)",[chat,nonce]));
await as(member);assert.equal((await rows('SELECT * FROM chat_messages WHERE conversation_id=$1',[chat])).length,1);
assert.equal((await rows('SELECT * FROM notifications WHERE chat_id=$1',[chat])).length,1);
await rows('SELECT read_chat($1,$2)',[chat,mid]);assert.equal((await rows('SELECT is_read FROM notifications WHERE chat_id=$1',[chat]))[0].is_read,true);
await assert.rejects(()=>rows("INSERT INTO chat_messages(conversation_id,author_id,body,nonce) VALUES($1,$2,'Bypass',gen_random_uuid())",[chat,member]));
await as(outsider);assert.equal((await rows('SELECT * FROM chat_messages WHERE conversation_id=$1',[chat])).length,0);
await assert.rejects(()=>rows('SELECT open_chat($1,null)',[chatProject]));
await assert.rejects(()=>rows("SELECT send_chat($1,'No',null,'{}',gen_random_uuid())",[chat]));
await as(owner);const dm=(await rows('SELECT open_chat(null,$1) id',[member]))[0].id;
await assert.rejects(()=>rows("SELECT send_chat($1,'Bad reply',$2,'{}',gen_random_uuid())",[dm,mid]));
await assert.rejects(()=>rows("SELECT send_chat($1,'Bad mention',null,ARRAY[$2::uuid],gen_random_uuid())",[dm,outsider]));
await as(admin);assert.equal((await rows('SELECT * FROM chat_conversations WHERE id=$1',[dm])).length,0);
await as(null);await rows('DELETE FROM project_members WHERE project_id=$1 AND user_id=$2',[chatProject,member]);
await as(member);assert.equal((await rows('SELECT * FROM chat_messages WHERE conversation_id=$1',[chat])).length,0);
await as(null);await rows('UPDATE projects SET is_archived=true WHERE id=$1',[chatProject]);
await as(owner);await assert.rejects(()=>rows("SELECT send_chat($1,'Archived',null,'{}',gen_random_uuid())",[chat]));
console.log('Chat: membership, private DMs, idempotency, replies, mentions, read receipts, revoked access and archived read-only passed.');
await as(null);await db.exec(read('postgres/021_task_acknowledgements.sql'));
await as(owner);
const ackProject=(await rows("SELECT create_workspace_project('Acknowledgement QA','',ARRAY[$1::uuid],true) id",[member]))[0].id;
const ackTask=(await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids) VALUES($1,'Acceptance QA',$2,ARRAY[$3::uuid,$2::uuid]) RETURNING id",[ackProject,owner,member]))[0].id;
const receipt=(await rows('SELECT id FROM task_acknowledgements WHERE task_id=$1 AND user_id=$2',[ackTask,member]))[0].id;
await assert.rejects(()=>rows('SELECT accept_task_assignment($1)',[receipt]));
await as(outsider);assert.equal((await rows('SELECT * FROM task_acknowledgements WHERE task_id=$1',[ackTask])).length,0);
await assert.rejects(()=>rows('SELECT accept_task_assignment($1)',[receipt]));
await as(null);
const assignment=(await rows("SELECT * FROM email_queue WHERE task_id=$1 AND user_id=$2 AND category='assignment' ORDER BY created_at DESC LIMIT 1",[ackTask,member]))[0];
assert.ok(assignment);const assignmentMail=await emailLogic.prepareEmail(db,assignment);assert.ok(assignmentMail.html.includes('/acknowledge/'+receipt));assert.ok(assignmentMail.html.includes('Accept task'));
await as(member);await rows('SELECT accept_task_assignment($1)',[receipt]);await rows('SELECT accept_task_assignment($1)',[receipt]);
assert.equal((await rows('SELECT status FROM tasks WHERE id=$1',[ackTask]))[0].status,'todo');
assert.equal((await rows("SELECT count(*) n FROM task_history WHERE task_id=$1 AND field='acknowledgement'",[ackTask]))[0].n,1);
await assert.rejects(()=>rows('UPDATE task_acknowledgements SET accepted_at=null WHERE id=$1',[receipt]));
await as(null);const receiptJobs=await rows("SELECT * FROM email_queue WHERE task_id=$1 AND category='task_accepted'",[ackTask]);assert.equal(receiptJobs.length,1);assert.equal(receiptJobs[0].user_id,owner);
const acceptanceMail=await emailLogic.prepareEmail(db,receiptJobs[0]);assert.ok(acceptanceMail.html.includes('Accepted by'));assert.ok(acceptanceMail.html.includes('1 of 2 accepted'));assert.equal(await emailLogic.prepareEmail(db,assignment),null);
...
await as(null);await db.exec(read('postgres/022_acknowledgement_source.sql'));
const cutoff=(await rows("SELECT now()::text stamp"))[0].stamp;
await rows("SELECT set_config('tasktracker.ack_cutoff',$1,false)",[cutoff]);
const queueBefore=(await rows('SELECT count(*) n FROM email_queue'))[0].n;
await db.exec(read('scripts/postgres/accept-existing-assignments.sql'));
assert.equal((await rows('SELECT count(*) n FROM email_queue'))[0].n,queueBefore);
assert.equal((await rows('SELECT acceptance_source FROM task_acknowledgements WHERE id=$1',[newer.id]))[0].acceptance_source,'administrative');
const historyBefore=(await rows('SELECT count(*) n FROM task_history'))[0].n;
await db.exec(read('scripts/postgres/accept-existing-assignments.sql'));
assert.equal((await rows('SELECT count(*) n FROM task_history'))[0].n,historyBefore);
await rows('UPDATE projects SET is_archived=false WHERE id=$1',[ackProject]);
const future=(await rows("INSERT INTO tasks(project_id,title,created_by,assignee_ids) VALUES($1,'New task after rollout',$2,ARRAY[$3::uuid]) RETURNING id",[ackProject,owner,member]))[0].id;
await db.exec(read('scripts/postgres/accept-existing-assignments.sql'));
