...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ summary?: string }> }) {
  ...
  const { data, error } = await supabase.from('tasks').select('id, project_id, title, status, priority, due_date, assignee:profiles!tasks_assignee_id_fkey(full_name,email), project:projects!inner(name,is_archived)').eq('project.is_archived', false).eq('is_archived',false).order('due_date', { nullsFirst: false });
