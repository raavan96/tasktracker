...
export async function taskPage(filters:TaskFilters,exportAll=false){return workspaceRead(async(db,user)=>{
 ...
 if(filters.mine)where.push('t.assignee_id='+bind(user)+'::uuid');
 if(filters.assignee&&filters.assignee!=='all')where.push(filters.assignee==='unassigned'?'t.assignee_id IS NULL':'t.assignee_id='+bind(filters.assignee)+'::uuid');
 ...
 const from=' FROM tasks t JOIN projects p ON p.id=t.project_id LEFT JOIN profiles a ON a.id=t.assignee_id LEFT JOIN profiles c ON c.id=t.created_by WHERE '+where.join(' AND ');
...
export async function reportData(f:ReportFilters,exportAll=false){
 ...
 const where=" WHERE e.occurred_at>=($1::date::timestamp AT TIME ZONE 'Asia/Kolkata') AND e.occurred_at<(($2::date+1)::timestamp AT TIME ZONE 'Asia/Kolkata') AND ($3::uuid IS NULL OR e.project_id=$3::uuid) AND ($4::uuid IS NULL OR e.assignee_id=$4::uuid) AND ($5::boolean OR (NOT t.is_archived AND NOT p.is_archived))";
 const from=' FROM completion_events e JOIN tasks t ON t.id=e.task_id JOIN projects p ON p.id=e.project_id LEFT JOIN profiles a ON a.id=e.assignee_id';
 ...
 const trend=(await db.query<{day:string;events:string}>("SELECT date_trunc('week',e.occurred_at AT TIME ZONE 'Asia/Kolkata')::date::text day,count(*) events"+from+where+' GROUP BY 1 ORDER BY 1',args)).rows;
 const overdue=Number((await db.query<{n:string}>("SELECT count(*) n FROM tasks t JOIN projects p ON p.id=t.project_id WHERE NOT t.is_archived AND NOT p.is_archived AND t.status NOT IN ('done','in_review') AND t.due_date<(now() AT TIME ZONE 'Asia/Kolkata')::date AND ($1::uuid IS NULL OR t.project_id=$1::uuid) AND ($2::uuid IS NULL OR t.assignee_id=$2::uuid)",[f.project||null,f.assignee||null])).rows[0].n);
