...
export type TableTask = { id: string; project_id: string; title: string; status: string; priority: string; due_date: string | null; assignee?: { full_name: string | null; email: string } | null; creator?: { full_name: string | null; email: string } | null; project?: { name: string } | null };
export default function TaskTable({ tasks, today, onOpen, initialSummary = 'all' }: { tasks: TableTask[]; today: string; onOpen?: (id: string) => void; initialSummary?: string }) {
  ...
  const filtered = tasks.filter(task => matchesSummary(task, summary, today) && `${task.title} ${task.assignee?.full_name || ''} ${task.assignee?.email || ''} ${task.project?.name || ''} ${task.creator?.full_name || ''} ${task.creator?.email || ''}`.toLowerCase().includes(search.toLowerCase()));
  function download() {
  ...
  return <div className="task-table-view space-y-4">
    ...
    <label className="inline-flex min-h-11 items-center gap-2 text-sm text-gray-600"><input type="checkbox" checked={showCreator} onChange={event => setShowCreator(event.target.checked)} />Show Created by column</label>
