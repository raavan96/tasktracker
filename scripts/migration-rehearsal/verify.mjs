...
try {
 await db.connect();
 for (const [table, source] of [['auth.users',data.users], ...Object.entries(data.rows).map(([k,v])=>['public.'+k,v])]) {
 ...
 const enabled = await db.query('SELECT count(*)::int n FROM auth.users WHERE NOT disabled OR password_hash IS NOT NULL');
 assert.equal(enabled.rows[0].n,0,'Unexpected usable imported login');
 console.log(JSON.stringify({ database: process.env.PGDATABASE, accounts: data.users.length, fieldComparisons: 'all imported fields match', visibilityAssertions: assertions, applicationTriggers: 'enabled', importedLogins: 'disabled' }));
