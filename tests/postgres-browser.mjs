...
try{
  ...
  for(const width of [375,430,1280]){await admin.setViewportSize({width,height:932});await admin.getByRole('button',{name:'List view',exact:true}).click();await admin.getByRole('button',{name:'Grid view',exact:true}).click();await expect.poll(()=>admin.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);await admin.screenshot({path:`/tmp/release5-controls-${width}.png`,fullPage:true});}
