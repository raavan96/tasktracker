...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{archive?:string;status?:string}>}) {
  ...
  return (
    <div className="space-y-6">
      ...
      <ProjectCollection projects={(projects||[]).filter(p=>p.is_archived===archived).filter(p=>query.status==='attention'?needsAttention(p):query.status==='ready'?readyToComplete(p):true).map(p=>({id:p.id,name:p.name,description:p.description,created_at:p.created_at,creator:creators.get(p.created_by)?.full_name||creators.get(p.created_by)?.email||'Unavailable',memberCount:p.project_members?.[0]?.count||0,totalTasks:p.tasks?.length||0,completedTasks:p.tasks?.filter(t=>t.status==='done').length||0}))}/>
    </div>
  );
}
