...
export function parseTaskForm(formData: FormData) {
  ...
  if (!['todo', 'in_progress', 'blocked', 'done'].includes(status)) return { error: 'Choose a valid status.' } as const;
  ...
  return { data: { title, description: text('description'), assignee_id: text('assigneeId') || null, priority: priority as TaskPriority, status: status as TaskStatus, due_date: dueDate || null } } as const;
}
