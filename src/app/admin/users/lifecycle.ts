...
export async function memberImpact(memberId:string){
 ...
 db.from('tasks').select('id,title,project_id,assignee_id,assignee_ids,created_by,status,recurrence,review_version,is_archived'),
 ...
 return {tasks:active.filter(t=>assignedIds(t).includes(memberId)&&t.status!=='done'),projects:activeProjects,members:members.data||[],people:people.data||[],events:events.data||[],createdProjects:activeProjects.filter(p=>p.created_by===memberId).length,pendingReviews:active.filter(t=>t.created_by===memberId&&t.status==='in_review'&&!assignedIds(t).includes(memberId)).length,recurring:active.filter(t=>(assignedIds(t).includes(memberId)||t.created_by===memberId)&&t.recurrence!=='none').length};
}
