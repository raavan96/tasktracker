...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ summary?: string }> }) {
  ...
  const [{ data, error }, people] = await Promise.all([supabase.from('tasks').select('id, project_id, assignee_id, created_by, title, status, priority, due_date, assignee:profiles!tasks_assignee_id_fkey(full_name,email), project:projects!inner(name,is_archived)').eq('project.is_archived', false).eq('is_archived',false).order('due_date', { nullsFirst: false }), supabase.from('profiles').select('id,full_name,email')]);
